...
// Gross earnings / trading profits before deductions. Dividends are added separately.
...
export function calculateNationalInsurance(
  ...
  const { primaryThreshold, secondaryThreshold, upperEarningsLimit, employeeRates, employerRates } =
    constants.nationalInsurance;
...
export function calculateSelfEmployedNI(
  ...
  const {
    class2WeeklyRate,
    class2SmallProfitsThreshold,
    class4LowerLimit,
    class4UpperLimit,
    class4Rates,
  } = constants.selfEmployedNI;
  ...
  const inFirstBand = Math.min(
    Math.max(0, profits - class4LowerLimit),
    class4UpperLimit - class4LowerLimit,
  );
  if (inFirstBand > 0) {
...
export function calculateStudentLoanRepayments(
  ...
  nonPostgradPlans.forEach((plan, index) => {
    ...
    if (index === nonPostgradPlans.length - 1) {
      ...
      amount =
        income <= thresholds[plan]
          ? 0
          : (Math.min(income, thresholds[nonPostgradPlans[index + 1]]) - thresholds[plan]) *
            defaultRate;
    }
  ...
  const roundedNonPostgradTotal = Math.floor(nonPostgradTotal + 1e-9);
  let adjustment = nonPostgradTotal - roundedNonPostgradTotal;
  ...
  if (studentLoanPlans.includes('postgrad')) {
    ...
    const amount =
      income <= thresholds['postgrad']
        ? 0
        : Math.floor((income - thresholds['postgrad']) * postgradRate);
    total += amount;
...
// NI applies published weekly/monthly thresholds to each period independently.
// Fortnightly thresholds are twice weekly amounts. Uneven earnings can change
// the total compared with an annual estimate; historical mid-year rate changes
...
export function calculatePeriodNI(
  ...
  const { primaryThreshold, secondaryThreshold, upperEarningsLimit, employeeRates, employerRates } =
    constants.nationalInsurance;
  // HMRC publishes whole-pound weekly/monthly NI thresholds. Fortnightly
  // thresholds are twice the weekly values, not annual thresholds / 26.
  const periodThreshold = (annual: number) =>
    periodsPerYear === 26 ? Math.round(annual / 52) * 2 : Math.round(annual / periodsPerYear);
  const firstThreshold = periodThreshold(employer ? secondaryThreshold : primaryThreshold);
  const upperLimit = periodThreshold(upperEarningsLimit);
  const rates = employer ? employerRates : employeeRates;
  ...
  return { total: Math.round((total + Number.EPSILON) * 100) / 100, breakdown };
}
...
export function calculatePeriodSL(
  ...
  if (nonPostgradPlans.length > 0) {
    ...
    const periodThreshold = Math.floor((thresholds[plan] / periodsPerYear) * 100) / 100;
  ...
  if (studentLoanPlans.includes('postgrad')) {
    const periodThreshold = Math.floor((thresholds['postgrad'] / periodsPerYear) * 100) / 100;
...
export function calculateChildBenefits(
  ...
  if (childBenefits.mode === 'off' || childBenefits.numberOfChildren < 1) {
    return {
  ...
  return {
    childBenefits: {
      ...
      breakdown: showBenefits ? [{ rate: 'Child Benefits', amount: childBenefitAmount }] : [],
    },
    hicbc: hicbcCharge,
  };
}

// Relief is limited by relevant earnings, not by whether income exceeds the
// Personal Allowance. Eligible non-earners can receive relief on £3,600 gross.
export function grossManualPensionContributions(
  personalContribution: number,
  taxReliefAtSource: boolean,
  annualGrossIncome: number,
  _basicAllowance: number,
): number {
  if (!taxReliefAtSource) return personalContribution;
  const netRelievable = Math.min(personalContribution, Math.max(3600, annualGrossIncome) * 0.8);
  return personalContribution + netRelievable * 0.25;
}
...
export function calculateTaxes(inputs: TaxInputs): TaxCalculationResult {
  ...
  if (!constants) {
    throw new Error(
      `Unsupported tax year: "${inputs.taxYear}". Available: ${Object.keys(taxYears).join(', ')}`,
    );
  }
  ...
  const pensionContributions = inputs.pensionEnabled
    ? inputs.selfEmployed
      ? {
          ...
          salarySacrifice: 0,
          personal: inputs.pensionContributions.personal,
        }
  ...
  const autoEnrolmentAsSalarySacrifice = inputs.pensionEnabled
    ...
    : true;
  const taxReliefAtSource = inputs.pensionEnabled ? inputs.taxReliefAtSource : true;
  const studentLoan = inputs.studentLoanEnabled ? inputs.studentLoan : [];

  const annualGrossIncome = calculateAnnualGrossIncome(
    inputs.annualGrossSalary,
    inputs.annualGrossBonus,
  );

  // Resolve salary sacrifice to a £ amount (it can be entered as a % of gross income)
  const salarySacrificeAmount = Math.min(
  ...
  const autoEnrolmentEmployerContribution =
    autoEnrolmentBase * (pensionContributions.autoEnrolmentEmployer / 100);

  // Deduct auto enrolment contributions from gross income, but only if they are salary sacrificed
  if (autoEnrolmentAsSalarySacrifice) incomeAfterSalarySacrifice -= autoEnrolmentContribution;

  const dbEnabled = inputs.pensionEnabled && inputs.dbPensionEnabled && !inputs.selfEmployed;
  const dbContribution = dbEnabled
    ? Math.min(
        Math.max(
          0,
          incomeAfterSalarySacrifice -
        ...
        (inputs.annualGrossSalary * inputs.dbMemberContribution) / 100,
      )
    : 0;
  const dbAccrued =
    dbEnabled && inputs.dbAccrualDenominator > 0
      ? inputs.annualGrossSalary / inputs.dbAccrualDenominator
      : 0;
  const netPayContributions =
    (autoEnrolmentAsSalarySacrifice ? 0 : autoEnrolmentContribution) + dbContribution;
  const relevantEarnings = Math.max(0, incomeAfterSalarySacrifice - netPayContributions);
  ...
  const pensionReliefAtSource = grossedPersonalContribution - pensionContributions.personal;
  const relievablePersonal = taxReliefAtSource
    ? Math.min(grossedPersonalContribution, Math.max(3600, relevantEarnings))
    : Math.min(grossedPersonalContribution, relevantEarnings);
  const unrelievedPensionContributions = grossedPersonalContribution - relievablePersonal;
  const pensionPot: CalculationResult = {
    total:
      salarySacrificeAmount +
      autoEnrolmentContribution +
      autoEnrolmentEmployerContribution +
      grossedPersonalContribution,
    breakdown: [
      ...
      { rate: 'Personal payment', amount: pensionContributions.personal },
      { rate: 'Relief at source', amount: pensionReliefAtSource },
    ],
  };
  const dividends = inputs.annualGrossDividends;
  const earningsBeforePersonal = Math.max(0, incomeAfterSalarySacrifice - netPayContributions);
  const adjustedNetIncome = Math.max(0, earningsBeforePersonal + dividends - relievablePersonal);
  // RAS extends tax bands; it does not deduct the gross contribution from
  // taxable earnings. Doing both would grant the basic-rate relief twice.
  const nonDividendANI = Math.max(
    0,
    earningsBeforePersonal - (taxReliefAtSource ? 0 : relievablePersonal),
  );
  const dividendANI = dividends;
  ...
  const employerNI = inputs.selfEmployed
    ...
    : calculateNationalInsurance(incomeAfterSalarySacrifice, constants, true, false);
  ...
  if (inputs.pensionEnabled && inputs.employerNISavingsToPension && !inputs.selfEmployed) {
    const employerNIWithoutSacrifice = calculateNationalInsurance(
      annualGrossIncome.total,
      constants,
      true,
      false,
  ...
  // Calculate student loan repayments
  // Self Assessment includes unearned income when it exceeds £2,000.
  // Personal pension relief does not reduce the loan repayment earnings base.
  const loanIncome =
    (inputs.selfEmployed ? annualGrossIncome.total : incomeAfterSalarySacrifice) +
    (dividends > 2000 ? dividends : 0);
  const studentLoanRepayments = calculateStudentLoanRepayments(loanIncome, studentLoan, constants);
  ...
  // RAS widens the basic-rate band and shifts subsequent taxable-income
  // limits. The Scottish starter band stays unchanged.
  const extension = taxReliefAtSource ? relievablePersonal : 0;
  const extendBands = (bands: TaxYearConstants['incomeTax']['restOfUK'], scottish: boolean) =>
    bands.map(([rate, limit]): [number, number] => [
      rate,
      limit + (scottish && rate < 0.2 ? 0 : extension),
    ]);
  const taxConstants = {
    ...constants,
    incomeTax: {
      restOfUK: extendBands(constants.incomeTax.restOfUK, false),
      scotland: extendBands(constants.incomeTax.scotland, true),
    },
  };
  const incomeTax = calculateIncomeTax(taxableIncome, taxConstants, inputs.residentInScotland);
  ...
  const dividendTax = calculateDividendTax(
    dividendANI,
    taxableIncome,
    allowanceRemainingForDividends,
    taxConstants,
  );

  // Calculate child benefits and HICBC
  const childBenefitsResult = calculateChildBenefits(
    adjustedNetIncome,
    inputs.childBenefits,
    constants.childBenefitRates,
    constants.hicbc,
  );
  ...
  const combinedTaxes =
    ...
    dividendTax.total +
    employeeNI.total +
    studentLoanRepayments.total +
    childBenefitsResult.hicbc;
  ...
  const employeePensionContributions =
    salarySacrificeAmount +
    (autoEnrolmentAsSalarySacrifice ? autoEnrolmentContribution : 0) +
    netPensionDeductions;
  const combinedDeductions = combinedTaxes + employeePensionContributions;
  // Preserve negative cash flow so an unaffordable contribution is visible.
  const takeHomePay = annualGrossIncome.total + dividends - combinedDeductions;
  ...
  const thresholdIncome = Math.max(
    0,
    annualGrossIncome.total + dividends - relievablePersonal - netPayContributions,
  );
  ...
  const allowance = calculatePensionAnnualAllowance(
    thresholdIncome,
    adjustedIncome,
    constants.pensionAnnualAllowance,
  );
  ...
  return {
    ...
    unrelievedPensionContributions,
